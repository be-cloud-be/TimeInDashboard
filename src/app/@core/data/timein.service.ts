...
export class TimeInService {
    ...
    getChantierByActivite(chantier : string, month : string) {
        return this.http.get(`${url}/chantier_activites?chantier=${chantier}&month=${month}`)
            .map((response: Response) => {
                 return <IMonthDetailsActivitesLine[]>response.json();
             })
             .catch(this.handleError);
    ...
    getChantierByEmploye(chantier : string, month : string) {
        return this.http.get(`${url}/chantier_employes?chantier=${chantier}&month=${month}`)
            .map((response: Response) => {
